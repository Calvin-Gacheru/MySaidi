# type: ignore
"""
Saidi — FastAPI backend (calendar-aware edition)
    POST /chat  → Groq ReAct loop → { reply, updated_tasks, actions }
...
def _format_calendar(events: list[dict[str, Any]]) -> str:
    ...
    for ev in events:
        ...
        lines.append(f"- [{ev['id']}] {ev['title']} | {time_range}{flexible_tag}")
    return "\n".join(lines)


# ── Saidi's personality ───────────────────────────────────────────
SYSTEM_PROMPT = """
You are Saidi, Calvin's personal AI scheduling assistant and daily productivity companion.
...
class ChatRequest(BaseModel):
    ...
    active_tasks: list[dict[str, Any]] = Field(default_factory=list)   # list of CalendarEvent dicts
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "id": {
                        "type": "string",
                        "description": "Required for 'update' and 'remove'. The event's short id."
                    },
                    "title": {
                        "type": "string",
...
async def chat(req: ChatRequest):
    ...
    current_tasks: list[dict[str, Any]] = [dict(ev) for ev in req.active_tasks]
        ...
        for call in tool_calls:
            ...
            # ── manage_calendar ───────────────────────────────────
                ...
                if action == "add":
                    ...
                    event_id = str(tool_args.get("id", "")).strip()
                    target = _find_event_by_id(event_id, current_tasks)
                    if not target:
                        result_content = f"Error: No event found with id '{event_id}'."
                    else:
                        ...
                        result_content = f"Success: Updated event '{target['title']}' (id={event_id})"
                    ...
                    if not target:
                        ...
                        actions.append({"type": "remove_event", "payload": {"id": event_id, "title": target.get("title")}})
